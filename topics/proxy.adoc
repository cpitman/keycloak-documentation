...
proxy-address-forwarding::
  If enabled, the proxy will use the X-Forwarded-For and X-Forwarded-Proto headers to modify the request before processing. This is required when the proxy is behind a TLS terminating reverse proxy or load balancer. If enabled, you must ensure both headers are always set by the proxy. If not set, malicious clients can spoof their source IP. _OPTIONAL._. 
  Defaults to false.
